...
class RoomManager:
  ...
  # Join/leave
  ####
  def joinMulitRoom(self,rooms):
    """
    Join a room or return None if already joined.
    
    @type room: str or list
    ...
    for room in rooms:
      if room not in self._rooms:
        self._rooms_copy+=[room]
      else:
        pass
    if not len(self.rooms_copy) == 1 and not self.rooms_copy == "":
      self.onStartJoin(None, "ok")
    else:
      return None
  ...
  def leaveMulitRoom(self,rooms):
    """
    Join a room or return None if already joined.
    
    @type room: str or list
    @param room: rooms to leave
